...
| D8 | **Hyperlinks: projects only, as plain text.** The new template has zero `<w:hyperlink>` elements and zero hyperlink rels. `src/endpoint/hyperlinks.py` is **deleted**; a project's repo URL goes as plain text into the entry line's empty right-hand tab slot (where work entries put dates). Avoids minting synthetic `r:id`s, whose dangling-reference failure mode is documented as breaking the LibreOffice PDF render. |
